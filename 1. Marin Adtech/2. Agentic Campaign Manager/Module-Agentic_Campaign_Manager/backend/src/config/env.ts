...
export const config = {
  ...
  zilkrDispatcher: {
    ...
    baseUrl: process.env.DISPATCHER_URL !== undefined ? process.env.DISPATCHER_URL : (process.env.ZILKR_DISPATCHER_BASE_URL !== undefined ? process.env.ZILKR_DISPATCHER_BASE_URL : ''),
    accountId: process.env.ZILKR_DISPATCHER_ACCOUNT_ID !== undefined ? process.env.ZILKR_DISPATCHER_ACCOUNT_ID : '',
    publisher: process.env.ZILKR_DISPATCHER_PUBLISHER !== undefined ? process.env.ZILKR_DISPATCHER_PUBLISHER : 'google',
    timeout: parseInt(process.env.ZILKR_DISPATCHER_TIMEOUT || '10000', 10),
  },
};
...
// Validate Zilkr Dispatcher configuration
if (!config.zilkrDispatcher.baseUrl && process.env.NODE_ENV === 'production') {
  console.warn('Warning: DISPATCHER_URL or ZILKR_DISPATCHER_BASE_URL must be set for Zilkr Dispatcher integration');
}

// TypeScript types for config structure
export interface ZilkrDispatcherConfig {
  baseUrl: string;
  accountId: string;
  publisher: string;
  timeout: number;
...
export interface Config {
  ...
  zilkrDispatcher: ZilkrDispatcherConfig;
}
