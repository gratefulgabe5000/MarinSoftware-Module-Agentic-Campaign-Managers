/**
 * Zilkr Dispatcher Type Validation Utilities
 *
 * This module provides validation functions for all Zilkr Dispatcher API request types.
...
/**
 * Validates a Zilkr campaign request object
 *
...
export function validateCampaignRequest(
  ...
  // Validate campaignBudget (resource reference - required by Google Ads API)
  if (!isNonEmptyString(request.campaignBudget)) {
    errors.push('campaignBudget is required and must be a non-empty string (resource reference)');
  }
...
/**
 * Validates a Zilkr ad request object (Responsive Search Ad)
 *
 * @param request - The ad request to validate
 * @returns ValidationResult with isValid flag and array of error messages
 */
export function validateAdRequest(request: ZilkrAdRequest): ValidationResult {
  const errors: string[] = [];
...
export function validateBatchOperation(
  ...
  if (!operation.resource) {
    ...
    switch (operation.resourceType) {
      case 'CAMPAIGN':
        resourceValidation = validateCampaignRequest(operation.resource as ZilkrCampaignRequest);
      ...
      case 'ADGROUP':
        resourceValidation = validateAdGroupRequest(operation.resource as ZilkrAdGroupRequest);
        if (!resourceValidation.isValid) {
          errors.push(...resourceValidation.errors.map(err => `resource.${err}`));
        }
        break;
      ...
      case 'AD':
        resourceValidation = validateAdRequest(operation.resource as ZilkrAdRequest);
        if (!resourceValidation.isValid) {
          errors.push(...resourceValidation.errors.map(err => `resource.${err}`));
        }
        break;
      ...
      case 'KEYWORD':
        resourceValidation = validateKeywordRequest(operation.resource as ZilkrKeywordRequest);
        if (!resourceValidation.isValid) {
          errors.push(...resourceValidation.errors.map(err => `resource.${err}`));
        }
        break;
