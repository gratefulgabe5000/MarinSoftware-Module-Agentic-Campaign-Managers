...
export class PatternExtractionController {
  ...
  queryPatterns = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const campaigns = await googleAdsService.queryCampaigns(
        accountId,
        dateRange
      );
      const adGroups = await googleAdsService.queryAdGroups(
        accountId,
        campaignIdsArray
      );
      const keywords = await googleAdsService.queryKeywords(
        accountId,
        campaignIdsArray,
        dateRange
      );
      const ads = await googleAdsService.queryAds(
        accountId,
        campaignIdsArray,
        dateRange
      );
